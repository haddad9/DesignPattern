...
public class WandController {
    ...
    public String controlWand(Model model) {
        System.out.println(service.getSpellNames());
        System.out.println(service.getMagicalEntities());
    ...
    @RequestMapping(path = "/save-recents", method = RequestMethod.POST)
    public String saveRecents(@RequestParam(value = "name")
                                          String name,
                              @RequestParam(value="spellQuantity") int spellQuantity) {
        service.registerSpells(name, spellQuantity);
        return "redirect:/control-wand";
    }
